...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: We assume that the caller has provided a valid mutable
    // reference to a `u32`. The caller must ensure that no other
    // references are active to the same memory location.
